...
%  input with one that has its power split evenly between in LG_10 and LG_00 modes, 
%  and with one that has 99% in LG_00 and 1% power in LG_10.
%
...
red_ratio_lg00_over_lg10 = 1;           % 20% of input power in LG_10, the rest in LG_00
...
for wave = 1:3 % loop through 
    ...
    % Scale such that the fields for LG_00 and LG_10 modes each have 1 watt power. You could alternatively do this numerically.
    LG_modes_00_and10(:,:,:,wave) = LG_modes_00_and10(:,:,:,wave) ./ ...
        (sqrt(0.5*epsilon_0*c)*diams_x(wave)*sqrt(log(2))); 

    % Show that each of the fields of the LG_00 and LG_10 modes have 1 watt power:
    mode_pwrs(:,wave) = dx * dy * 0.5 * epsilon_0 * c * ...
        [sum(abs(LG_modes_00_and10(:,:,1,wave)).^2, 'all'), sum(abs(LG_modes_00_and10(:,:,2,wave)).^2, 'all')];

    % Superposition of LG_00 and LG_p0 modes, with ratio between modes set by red_ratio_lg00_over_lg10 and 
    % phase between them set by lg00_lg10_phase. Total input power is 1 watt.
    field_xy_superposition = LG_modes_00_and10(:,:,1,wave).*sqrt(red_ratio_lg00_over_lg10/(1+red_ratio_lg00_over_lg10)) + ...
        exp(1i*lg00_lg10_phase)*LG_modes_00_and10(:,:,2,wave)*sqrt(1./((1+red_ratio_lg00_over_lg10)));
...
red1_red2_energy_input_vector = logspace(-10,-7.5,6);
