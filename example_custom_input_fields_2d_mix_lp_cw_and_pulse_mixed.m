% mixed cw and pulse case:
%   Red 1 is pulsed, blue is pulsed, red2 is cw
...
if calculate_curves
    ...
    lambda = [1000, 500, 1e3/3]*1e-9; % red1, red2, and blue wavelengths (in meters)
    num_energies = 5;
    red1_input_energy_list  = linspace(1e-8,1e-6,num_energies); % red 1 input energy - red 1 is pulsed
    red2_input_power        = 75;           % red 2 input power (W) - red 2 is cw
    ...
    nx = 96;                                % number of spatial grid points in the x (walkoff) direction
    ny = 96;                                % number of spatial grid points in the y (non-walkoff) direction
    nt = 101;                               % number of temporal points to include in the pulse
    durations = 3.5*[1,0,1]*1e-9;           % input pulse durations (fwhm s)
    lx = 0.4e-3;                            % extent of spatial grid in the x (walkoff) direction (m)
    ly = 0.4e-3;                            % extent of spatial grid in the y (non-walkoff) direction (m)
    spatial_supergaussian_coeffs    = [1,1,1];
    temporal_supergaussian_coeffs   = [1,1,1];
    pulse_delays = [0, 0, 0];               % delays (s)
    input_field_diameters = [51.7, 51.7; 39.7, 39.7; 1, 1]*1e-6;    % input beam diameters (in fwhm m) [red1 x-dir, red1 y-dir; red2 x-dir, red2 y-dir; blue x-dir, blue y-dir;] 
    position  = [0, 0; 0, 0; 0, 0]*1e-3;                % offset of center of beam (in walkoff direction & perpendicular to it) for red1, red2, and blue fields (m)
    beam_rocs = [5.57, 5.57; 13.3, 13.3; 20, 20]*1e-3;        % radius of curvature for (x&y planes) for red1, red2, and blue waves (m)
    ...
    % Convert irradiance into field (still corresponds to 1 W power)
    ...
    temporal_gaussian_vec_red2 = 1*size(t_vec);
...
%     norm_temporal_gaussian_vec_red2 = temporal_gaussian_vec_red2 ./ (tstep*trapz(temporal_gaussian_vec_red2));
    ...
    field_red2_xyt_norm = field_red2_xy_norm.*curvature_phase_red2;
...
%         field_red2_xyt_norm(:,:,K) = field_red2_xyt_norm(:,:,K);
    ...
    inputs.mix_2d_lp_ref_inds       = [1.655,1.677,1.670];  % refractive indices
    ...
    inputs.mix_2d_lp_pulseenergy    = [red1_input_energy_list(1),red2_input_power,blue_energy];        % input powers (in watts) or pulse energies (in joules)
    ...
    output_red1 = cell(size(red1_input_energy_list)); % pre-allocate a cell array to store the contents of the red1 output files in
    output_red2 = cell(size(red1_input_energy_list)); % pre-allocate a cell array to store the contents of the red2 output files in
    output_blue = cell(size(red1_input_energy_list)); % pre-allocate a cell array to store the contents of the blue output files in
    output_red1_energies = zeros(size(red1_input_energy_list)); % pre-allocate a vector for red1 output energies as fcn of input pulse energy
    output_red2_energies = zeros(size(red1_input_energy_list)); % pre-allocate a vector for red2 output energies as fcn of input pulse energy
    output_blue_energies = zeros(size(red1_input_energy_list)); % pre-allocate a vector for blue output energies as fcn of input pulse energy
    ...
    for K = 1:length(red1_input_energy_list) % loop over each element in the vector of input pulse energies
        energies = [red1_input_energy_list(1,K),red2_input_power,blue_energy];
        ...
        fprintf(1,'Run %i of %i - input red1 energy: %.4g J, input red2 power: %.4g W, output red1 energy: %.4g J, output blue energy: %.4g J\n',K,length(red1_input_energy_list),red1_input_energy_list(K),red2_input_power,output_red1_energies(K),output_blue_energies(K));
    end
end
...
% plot output powers vs red1 input energies
ph = plot(ax, red1_input_energy_list(1,:)*1e6, output_red1_energies*1e6,'r-',...
