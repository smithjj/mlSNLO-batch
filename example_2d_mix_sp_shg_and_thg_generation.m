...
red1_red2_energy_vec = linspace(0.05, 1, 16)*10e-6; % energy in Joules
...
shg_inputs.mix_2d_sp_pulse_chirps   = [0,0,0;0,0,0;0,0,0];      % Linear chirp of input pulse (in THz/ps) (three orders of chirp, in THz/ps, THz/ps^2, and THz/ps^3; one wave for each row).
shg_inputs.mix_2d_sp_beam_diameters = [0.885,0.885,0.885; 1.00,1.00,1.00].';    % input beam diameter (in fwhm mm), specified in walkoff direction, and perpendicular to walkoff direction (one wave per row)
...
shg_inputs.mix_2d_sp_rad_curv       = [1e9,1e9,1e9;1e9,1e9,1e9].';              % input beam radii of curvature (in mm in air) for [red1,red2,blue] and [in the walkoff direction;parallel to the walkoff direction]
shg_inputs.mix_2d_sp_nt             = 64;                       % number of time points to consider
shg_inputs.mix_2d_sp_nxny           = [64,64];                  % number of spatial grid points in the walkoff direction, and perpendicular to walkoff direction
shg_inputs.mix_2d_sp_crystal_length = 7.5;                      % length of nonlinear crystal (in mm)
shg_inputs.mix_2d_sp_lx_ly          = [3.5,3.5];                % size of spatial grid (in mm) in walkoff direction, and perpendicular to walkoff direction
...
thg_inputs.mix_2d_sp_beam_diameters = [0.885,0.885,0.885;0.885,0.885,0.885].';  % input beam diameter (in fwhm mm), specified in walkoff direction, and perpendicular to walkoff direction (one wave per row)
...
thg_inputs.mix_2d_sp_nt             = 64;                       % number of time points to consider
thg_inputs.mix_2d_sp_nxny           = [64,64];                  % number of spatial grid points in the walkoff direction, and perpendicular to walkoff direction
thg_inputs.mix_2d_sp_crystal_length = 7.5;                      % length of nonlinear crystal (in mm)
thg_inputs.mix_2d_sp_lx_ly          = [3.5,3.5];                % size of spatial grid (in mm) in walkoff direction, and perpendicular to walkoff direction
...
for K = 1:length(red1_red2_energy_vec) % loop over each element in the vector of input pulse energies
    ...
    % call the modified SNLO 2D-mix-SP function with the problem set,
    ...
    % In the type I shg stage, we split the red input energy equally between red1 and
    % red2. Now when we combine the red1 and red2 fields to form the new red1 input we sum
    % the red1 and red2 output fields and multiply by 1/sqrt(2). If the first stage is not
    % type 1 SHG, this is irrelevant.
    thg_input_fields.field_red1_xyt = sqrt(0.5) * (field_red1_xyt + field_red2_xyt);    % 1064 nm
    thg_input_fields.field_red2_xyt = field_blue_xyt;                                   % 532 nm
    thg_input_fields.field_blue_xyt = field_blue_xyt*1e-9;                              % 354.7nm starts with 0 energy, but setting its value to be just 0 can cause problems?
