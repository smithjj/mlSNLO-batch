...
if calculate_curves
    ...
    inputs.opg_2d_nt = 1024;                           % number of time points to consider
    ...
    clear problem
    spectra_s = cell(size(delta_k_list)); % pre-allocate cell array to load spectra from file into
    spectra_i = cell(size(delta_k_list)); % pre-allocate cell array to load spectra from file into
    for K = 1:length(delta_k_list)
        ...
        problem(K).opg_2d_deltak = delta_k_list(K); % modify this problem's phase mismatch to be an element of the vector of phase mismatches generated earlier
        ...
        spectra_s{K} = load('OPG_BEAM_3WS.DAT'); % load the red1 spectra file
        spectra_s{K}(:,1) = spectra_s{K}(:,1)*1E-12; % convert frequencies from Hz to THz
        spectra_s{K}(:,2) = spectra_s{K}(:,2)./max(spectra_s{K}(:,2)); % normalize power spectral densities to be maximum of 1
        spectra_i{K} = load('OPG_BEAM_3WI.DAT'); % load the red2 spectra file
        spectra_i{K}(:,1) = spectra_i{K}(:,1)*1E-12; % convert frequencies from Hz to THz
        spectra_i{K}(:,2) = spectra_i{K}(:,2)./max(spectra_i{K}(:,2)); % normalize power spectral densities to be maximum of 1
...
for K = 1:length(delta_k_list) % loop over vector of phase mismatches 
    ph(K) = plot(ax,spectra_s{K}(:,1),spectra_s{K}(:,2)); % plot output power spectral density as function of frequency in THz
    ...
    bx1(K) = spectra_s{K}(find(spectra_s{K}(:,2)>1E-4,1,'first'),1); % calculate lowest frequency with substantial power (>0.01% maximum power)
    bx2(K) = spectra_s{K}(find(spectra_s{K}(:,2)>1E-4,1,'last'),1);  % calculate highest frequency with substantial power (>0.01% maximum power)
end
